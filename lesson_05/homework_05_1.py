...
filtered_cars = [
    ...
    if details[1] >= search_criteria[0] and details[2] >= search_criteria[1] and details[-1] <= search_criteria[-1]]
print(filtered_cars)  # попрінтила усі машини за цим фільтром
# тут підгледіла лямбду - https://docs.python.org/3/howto/sorting.html
# щоб витягти з кожного вкладеного кортежа ціну машини і посортувати за нею
# якщо є простіший спосіб то натякни, бо я щось не можу придумати
sorted_by_price_cars = sorted(filtered_cars, key=lambda car_price: car_price[1][-1], reverse=False)
car_tuple_len = len(sorted_by_price_cars)
for car in sorted_by_price_cars[:5]:  # від 0 до 5 не включно
    print(car)
